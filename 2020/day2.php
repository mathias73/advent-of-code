...
foreach ($passwordList as $password) {
    ...
    $explodePass = explode(":", $password);
    $explodeRgx1 = explode("-", $explodePass[0]);
    $explodeRgx2 = explode(" ", $explodeRgx1[1]);

    $explodePass1 = trim($explodePass[1]);
    $passSplit = str_split($explodePass1);

    if (strpos($explodePass[1], $explodeRgx2[1])) {
        if (substr_count($explodePass[1], $explodeRgx2[1]) >= $explodeRgx1[0]) {
            if (substr_count($explodePass[1], $explodeRgx2[1]) <= $explodeRgx2[0]) {
                $count++;
            }
        }
    }

    if (($passSplit[$explodeRgx1[0] - 1] === $explodeRgx2[1]) != ($passSplit[$explodeRgx2[0] - 1] === $explodeRgx2[1])) {
        $count1++;
    }
